...
import {
  createWorkspaceFinanceRecord,
  getWorkspaceFinanceDashboard,
...
export async function GET() {
  try {
    ...
    const result = await getWorkspaceFinanceDashboard({
...
export async function POST(request: Request) {
  try {
    ...
    const result = await createWorkspaceFinanceRecord(
      {
        workspaceId: context.workspace.id,
        userId: context.user.clerkUserId,
        currency: context.workspace.base_currency,
